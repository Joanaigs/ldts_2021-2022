...
public class GameModel implements Model{
    ...
    public void updateAux(long deltatime){
        ...
        if (collideWithWalls(new Collider(next, pacman.getCollider().getWidth(), pacman.getCollider().getHeight()))) {
            ...
            SmallCoin temp1_coin = null;
            boolean found1 = false;
            for( SmallCoin smallCoin: map.getSmallCoins()){
                if(smallCoin.getCollider().colision(new Collider(next, pacman.getCollider().getWidth(), pacman.getCollider().getHeight()))){
                    temp1_coin = smallCoin;
                    found1 = true;
                    pacman.increaseScore(SmallCoin.SmallCoinValue);
                }
            }
            if (found1)
                map.getSmallCoins().remove(temp1_coin);

            else{
                PowerCoin temp2_coin = null;
                boolean found2 = false;
                for( PowerCoin powerCoin: map.getPowerCoins()){
                    if(powerCoin.getCollider().colision(new Collider(next, pacman.getCollider().getWidth(), pacman.getCollider().getHeight()))){
                        temp2_coin = powerCoin;
                        found2 = true;
                        pacman.increaseScore(PowerCoin.PowerCoinValue);
                    }
                }
                if(found2) {
                    map.getPowerCoins().remove(temp2_coin);
